...
class Nestbox
{
     ...
     */
    public static function generate_column_list_from_params(string $table, array $params): array
    {
     ...
     * @throws InvalidColumnException
     */
    public function generate_where_clause($table, $where, $conjunction): array
    {
        $clause = [];
        $params = [];
    ...
    public function update(string $table, array $updates, array $where = [], string $conjunction = "AND"): int|bool
    {
        ...
        foreach ($where as $column => $value) {
            ...
            if (in_array($whereKey, $setParams) or $this->valid_schema($table, $whereKey)) {
                ...
                while (in_array($whereKey) or $this->valid_schema($table, $whereKey)) {
    ...
    public function delete(string $table, array $where, string $conjunction = "AND", bool $deleteAll = false): int|bool
    {
        ...
        if (!$this->query_execute("DELETE FROM $table WHERE $where;", $params)) return false;
    ...
    public function load_table(string $table, string|array $data, bool $truncate = false): int
    {
        ...
        return $this->insert(table: $table, params: $data, updateOnDuplicate: true);
    }
